...
public  function ChartOfAccountDelete($ChartOfAccountID)
{
  ...
  $chart = DB::table('chartofaccount')->where('ChartOfAccountID',$ChartOfAccountID)->get();

  if($chart[0]->Status=='LOCK')
  {
    return redirect ('ChartOfAcc')->with('error', 'Unable to delete this chart of account. please contact the software company.')->with('class','danger');    
  }
  else{
