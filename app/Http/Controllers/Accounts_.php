...
public  function PettyCashCreate()
    {
      ...
      ////////////////////////////END SCRIPT ////////////////////////////////////////////////
           ...
           $chartofaccount = DB::table('chartofaccount')->where('Category','CASH')->get();  
           $chartofaccount1 = DB::table('chartofaccount')->where('Level',3)->get();  
           $supplier = DB::table('supplier')->get();  
           $vhno = DB::table('invoice_master')->select(DB::raw('max(InvoiceMasterID)+1 as VHNO'))->get();  
 
 
 
    return view ('pettycash_create',compact('voucher_type','items','supplier','vhno','pagetitle','chartofaccount','chartofaccount1'))->with('error', 'Logout Successfully.')->with('class','success');
    }
...
$iddd= DB::table('pettycash_detail')->insert($invoice_det);
    ...
    return view ('pettycash_edit',compact('chartofaccount','chartofaccount1','pettycash_master','pettycash_detail','pagetitle'));
    }
...
{   
    ...
    $id1 = DB::table('pettycash_detail')->where('PettyMstID',$id)->delete();
    
    return redirect('PettyCash')->with('error','Record Deleted')->with('class','success');
}
   ...
   public  function JV()
    {
           ...
           $chartofaccount=DB::table('chartofaccount')->where('Level',3)->get();
...
$chartofaccount1 = DB::table('chartofaccount')->where('Level',3)
 ...
 $chartofaccount = DB::table('chartofaccount')->where('Level',3)
               ->get();  
...
///////////////////////USER RIGHT & CONTROL ///////////////////////////////////////////    
 ...
 $invoice_mst = DB::table('v_invoice_master')->where('InvoiceMasterID',$request->InvoiceMasterID)->get();
 $invoice_det = DB::table('v_invoice_detail')->where('InvoiceMasterID',$request->InvoiceMasterID)->get();

   // return View ('invoice_pdf',compact('invoice_type','items','supplier','vhno','invoice_mst','invoice_det'));

 $filename = $invoice_mst[0]->InvoiceCode.'-'.$invoice_mst[0]->Date.'-PartyCode-'.$invoice_mst[0]->PartyID.'.pdf';
  ...
  // return $pdf->download($filename.'.pdf');
       ...
       $data["email"] = $request->Email;
        $data["title"] = "Invoice No : ". $invoice_mst[0]->InvoiceCode;
        $data["body"] = "Invoice in PDF ";
        $data["InvoiceMasterID"] = $request->InvoiceMasterID;
...
return redirect('Invoice')->with('error','Mail Sent Successfully')->with('class','success');
...
// search in detail table
     public function ajax_invoice2(Request $request)
...
<a target="_blank"  href="'.URL('/InvoiceView/'.$row->InvoiceMasterID).'"><i class="font-size-18 mdi mdi-eye-outline align-middle me-1 text-secondary"></i></a> 
...
<a href="'.URL('/InvoicePDF/'.$row->InvoiceMasterID).'"><i class="font-size-18 mdi mdi-file-pdf-outline align-middle me-1 text-secondary"></i></a> 


  
<a href="#" id="'.$row->InvoiceMasterID.'" onclick="openModal('.$row->InvoiceMasterID.')"><i class="font-size-18 me-1 bx bx bx-envelope  align-middle me-1 text-secondary"></i></a> 
...
$chartofaccount = DB::table('chartofaccount')->whereIn('Category',['CASH','BANK','CARD'])->get(); 
 ...
 $invoice_det = DB::table('invoice_detail')->where('InvoiceMasterID',$id)->get();
    ...
    return view ('invoice_edit',compact('invoice_type','items','supplier','vhno','invoice_mst','invoice_det','party','saleman','customer','chartofaccount','company'))->with('error', 'Logout Successfully.')->with('class','success');
    }
...
$where = array_add( $where, 'SupplierID', $request->PartyID);
