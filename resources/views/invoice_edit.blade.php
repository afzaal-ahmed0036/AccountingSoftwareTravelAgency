                    ...
                    <div class="row">
                        <div class="col-6"> <img src="{{ asset('documents/' . $company[0]->Logo) }}" alt=""
                            ...
                            <div class="col-6">
                                ...
                                <select name="PartyID" id="PartyID" class="form-select select2 mt-5" name="PartyID"
                                    required="">
                                    <?php foreach ($customer as $key => $value): ?>
                                    <option value="{{ $value->PartyID }}"
                                        {{ $value->PartyID == $invoice_mst[0]->PartyID ? 'selected=selected' : '' }}>
                                        {{ $value->CategoryCode }} - {{ $value->PartyName }}</option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                        </div>
                                        ...
                                        <div class="col-sm-9">
                                            <select name="PaymentMode" id="PaymentMode" class="form-select">
                                                ...
                                                <option value="{{ $value->ChartOfAccountID }}"
                                                    {{ $invoice_mst[0]->PaymentMode == $value->ChartOfAccountID ? 'selected=selected' : '' }}>
                                    ...
                                    <div class="mb-1 row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="SalemanID" id="SalemanID" class="form-select">
                                                ...
                                                <option value="{{ $value->SalemanID }}"
                                                    {{ $value->SalemanID == $invoice_mst[0]->UserID ? 'selected=selected' : '' }}>
                                                    {{ $value->SalemanName }}</option>
                                                <?php endforeach ?>
                            ...
                            <table>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($invoice_det as $key => $value1)
                                        <?php $no = $key + 1; ?>
                                        <tr class="p-3">
                                            <td class="p-1 bg-light borde-1 border-light"><input class="case"
                                                    type="checkbox" /></td>
                                            <td>

                                                <select name="ItemID0[]" id="ItemID0_{{ $no }}"
                                                    class="form-select form-control-sm   changesNoo">
                                                ...
                                                <input type="hidden" name="ItemID[]"
                                                    id="ItemID_{{ $no }}" value="{{ $value1->ItemID }}">
                                            </td>
                                            <td> <select name="SupplierID[]" id="SupplierID_{{ $no }}"
                                                    class=" form-select changesNo select2"
                                                    onchange="ajax_balance(this.value);">
                                                    @foreach ($party as $key => $value)
                                                        <option value="{{ $value->PartyID }}"
                                                            {{ $value->PartyID == $value1->SupplierID ? 'selected=selected' : '' }}>
                                                            {{ $value->CategoryCode }}-{{ $value->PartyName }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="text" name="RefNo[]" id="RefNo_{{ $no }}"
                                            ...
                                            <td>
                                                <input type="text" name="PaxName[]"
                                                    id="PaxName_{{ $no }}" class=" form-control changesNo"
                                                    autocomplete="off" value="{{ $value1->PaxName }}">
                                            </td>
                                            <td>
                                                <input type="text" name="PNR[]" id="PNR_{{ $no }}"
                                                    class=" form-control changesNo" autocomplete="off"
                                                    value="{{ $value1->PNR }}">
                                            </td>
                                            <td>
                                                <input type="text" name="Sector[]"
                                                    id="Sector_{{ $no }}" class=" form-control changesNo"
                                                    autocomplete="off" value="{{ $value1->Sector }}">
                                            </td>
                                            <td>
                                                <input type="number" name="Fare[]" id="Fare_{{ $no }}"
                                                    class=" form-control changesNo" autocomplete="off"
                                            ...
                                            <td>
                                                <input type="number" name="IPVAT[]" id="IPVAT_{{ $no }}"
                                                    class=" form-control changesNo" autocomplete="off"
                                                    onkeypress="return IsNumeric(event);" ondrop="return false;"
                                                    onpaste="return false;" step="0.01"
                                                    value="{{ $value1->IPVAT }}">
                                            ...
                                            <td>
                                                <input type="number" name="TaxAmount[]"
                                                    id="quantity_{{ $no }}" class=" form-control "
                                                    autocomplete="off" onkeypress="return IsNumeric(event);"
                                                    ondrop="return false;" onpaste="return false;" step="0.01"
                                                    value="{{ $value1->Taxable }}">
                                            </td>
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        <input type="hidden" class="form-control" id="subTotal" name="subTotal"
                                            placeholder="Subtotal" onkeypress="return IsNumeric(event);"
                                ...
                                <div class="form-group mt-1">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" id="amountPaid" name="amountPaid"
                                            placeholder="Amount Paid" onkeypress="return IsNumeric(event);"
                                            ondrop="return false;" onpaste="return false;" step="0.01"
    ...
    <script>
        ...
        $(".addmore").on('click', function() {
            ...
            html += '<td><select name="ItemID0[]" id="ItemID0_' + i +
                '" class="form-select changesNoo"> @foreach ($items as $key => $value) <option value="{{ $value->ItemID }}|{{ $value->Percentage }}">{{ $value->ItemCode }}-{{ $value->ItemName }}-{{ $value->Percentage }}</option>@endforeach</select><input type="hidden" name="ItemID[]" id="ItemID_' +
                i + '"></td>';
            ...
            html += '<td><select name="SupplierID[]" id="SupplierID_' + i +
                '"  onchange="ajax_balance(this.value);" class=" form-select select2">@foreach ($supplier as $key => $value) <option value="{{ $value->SupplierID }}">{{ $value->SupplierName }}</option>@endforeach</select></td>';
            ...
            html += '<td><input type="text" name="Fare[]" id="Fare_' + i +
            ...
            html += '<td><input type="text" name="Service[]" id="Service_' + i +
                '" class="form-control " autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
            html += '<td><input type="text" name="OPVAT[]" id="OPVAT_' + i +
            ...
            html += '<td><input type="text" name="IPVAT[]" id="IPVAT_' + i +
                '" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
            html += '<td><input type="text" name="TaxAmount[]" id="quantity_' + i +
                '" class="form-control " autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
            html += '<td><input type="text" name="Discount[]" id="discount_' + i +
                '" class=" form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
            html += '<td><input type="text" name="ItemTotal[]" id="total_' + i +
                '" class="form-control totalLinePrice" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
            html += '</tr>';
            $('table').append(html);
        ...
        $(document).on('change keyup blur', '.changesNo', function() {
            ...
            $('#total_' + id[1]).val((parseFloat(Fare) + parseFloat(Service) + parseFloat(OPVAT) + parseFloat(
                IPVAT) - parseFloat(Discount)).toFixed(2));
