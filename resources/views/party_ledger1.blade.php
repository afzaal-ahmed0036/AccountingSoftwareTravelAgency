  ...
  <div class="card">
      <div class="card-body">
          ...
          @foreach ($journal as $key =>$value)
           ...
           <td >{{$value->Narration}} - >{{$value->ChartOfAccountName}}
           <div style="font-size: 11px;"> <?php 
            if($value->JournalType=='SI'){

            $invoice_detail = DB::table('invoice_detail')->where('InvoiceMasterID',$value->InvoiceMasterID)->get();  
            echo "<br>---------INVOICE DETAILS------------<br>";
            foreach($invoice_detail as $key => $value1)
            {

              echo "PNR: ".$value1->PNR . ' PAX Name: '. $value1->PaxName." Tkt# ." .$value1->RefNo."<br>";
